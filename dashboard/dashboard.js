/**
 * Mavis Stock Tracker — Dashboard.js v32.26 (2026-09-25)
  ...
  const App = {
    ...
    renderPnlTrendChart(series) {
      ...
      if (recent5El) {
        ...
        const rows = [
          // row 1: 日期行 (label 留空)
          '<div class="pnl-recent-label"></div>' + last5.map(s => `<div class="pnl-recent-cell"><div class="pnl-recent-date">${s.trade_date.substring(5)}</div></div>`).join(''),
          // row 2: 实 (label = "实")
          '<div class="pnl-recent-label">实</div>' + last5.map((s, i) => {
