/**
 * Mavis Stock Tracker — Dashboard.js v32.29 (2026-09-25)
...
(function () {
  ...
  const App = {
    ...
    renderPnlTrendChart(series) {
      ...
      // v32.29: 最近 3 个日期速览 (4 列 × 4 行: 1 label + 3 data)
      // 行顺序: 日期 / 实 / gap / 总 (user 反馈把 gap 放中间)
      ...
      if (recent5El) {
        ...
        // 4 行 × 4 列 (1 label + 3 data), grid auto-flow row 按顺序填
        const rows = [
          // row 1: 日期行 (label 留空)
          '<div class="pnl-recent-label"></div>' + last3.map(s => `<div class="pnl-recent-cell"><div class="pnl-recent-date">${s.trade_date.substring(5)}</div></div>`).join(''),
          // row 2: 实
          '<div class="pnl-recent-label">实</div>' + last3.map((s, i) => {
            const realIdx = series.length - 3 + i;
            const r = realizedPnls[realIdx] || 0;
            return `<div class="pnl-recent-cell"><div class="pnl-recent-val ${r >= 0 ? 'up' : 'down'}">${fmt(r)}</div></div>`;
          }).join(''),
          // row 3: gap (v32.29: 移到中间, 跟 chart 视觉对应)
          '<div class="pnl-recent-label">gap</div>' + last3.map((s, i) => {
            const realIdx = series.length - 3 + i;
            const r = realizedPnls[realIdx] || 0;
            const t = totalPnls[realIdx] || 0;
            const f = t - r;
            return `<div class="pnl-recent-cell"><div class="pnl-recent-val ${f >= 0 ? 'up' : 'down'}">${fmt(f)}</div></div>`;
          }).join(''),
          // row 4: 总
          '<div class="pnl-recent-label">总</div>' + last3.map((s, i) => {
            const realIdx = series.length - 3 + i;
            const t = totalPnls[realIdx] || 0;
            return `<div class="pnl-recent-cell"><div class="pnl-recent-val ${t >= 0 ? 'up' : 'down'}">${fmt(t)}</div></div>`;
          }).join(''),
        ];
        recent5El.innerHTML = rows.join('');
      }
    },
